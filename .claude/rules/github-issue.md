...
開発の作業単位を GitHub issue で管理する（issue 駆動開発）。ブランチ運用・命名は [`git.md`](git.md)、PR 作成/承認のショートカットは [`shortcuts.md`](shortcuts.md) に従う。
...
- **ブランチとの紐付け**: ブランチ名は GitHub Flow の命名規約（`feature/*`, `fix/*`, `chore/*` 等。詳細は `git.md`）に従い、対応する issue を PR で参照する。
